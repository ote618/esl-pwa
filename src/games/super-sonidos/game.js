...
export function startSuperSonidos (root, { testMode = false, showTail = false } = {}) {
  ...
  function draw(){
   ...
   for(const b of G.blocks){const px=b.cx*T,py=b.cy*T-Math.round(b.bp*6);b.bp*=.8;if(b.bd>0)b.bd--;
    // Gold = waiting to be hit. Dull gold = already hit, and stays that way so
    // a child can see at a glance which boxes are done. No red: there is
    // nothing here to get wrong. The 26-frame knock is only for a box from a
    // round that has not come round yet.
    ctx.fillStyle=b.hit?'#B0873A':b.bd>0?'#E8C46A':'#FFC93C';ctx.fillRect(px,py,T,T);
